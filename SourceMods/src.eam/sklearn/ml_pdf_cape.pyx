...
def cape_pdf(pcols, temp, pres, qv):
    ...
    ####sklearn
    model2 = xgb.XGBRegressor(n_estimators=60, max_depth=3, n_jobs=20)
    model2.load_model("/pscratch/sd/z/zhangtao/E3SMv2_1/F2010.ne30pg2_EC30to60E2r2_ML_BCU/case_scripts/SourceMods/src.drv/sklearn/model.json")

    print("after load model")

    tqp_data = tqp_data.reshape(pcols,-1)
    extra_data = extra_data.reshape(pcols, -1)
    data = np.concatenate([tqp_data,extra_data],axis=1)

    print("after concatenate data")

    BCU_trig = model2.predict(data).astype(np.float64)
    expectation_value = model2.predict(data).astype(np.float64)
