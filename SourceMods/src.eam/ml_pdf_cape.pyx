...
#    print(f'Time in Python: import = {time2-time1:.2f}s')
    ...
    time1 = time.time()
    file_path = "/global/homes/z/zhangtao/cfs_e3sm/ML4E3SM_new/models/Metoffice/"
    ...
    CAPE_model = torch.jit.load(f'{file_path}/model_1124/model_torch_E3SM_ML_PDF_CAPE.pt')
    BCU_model = torch.jit.load(f'{file_path}/model_1124/model_torch_E3SM_ML_BCU.pt')
    ...
    #CAPE_model = torch.load("/pscratch/sd/z/zhangtao/E3SMv2_1/F2010.ne30pg2_EC30to60E2r2_ML_Test_Framework/case_scripts/SourceMods/src.eam/pytorch/pytorch_model")
    #BCU_model = torch.load("/pscratch/sd/z/zhangtao/E3SMv2_1/F2010.ne30pg2_EC30to60E2r2_ML_Test_Framework/case_scripts/SourceMods/src.eam/pytorch/pytorch_model")
...
#    print(f'Time in Python: convert double ** to numpy  = {time2-time1:.2f}s')
    ...
    print("before cape_pdf")
    BCU_trig, CAPE = cape_pdf(pcols, temp, pres, qv)
    print("in calc_trigger",BCU_trig, CAPE)
    cape = np.concatenate([BCU_trig,CAPE])
    print("after cape_pdf")
    print(cape)
...
def cape_pdf(pcols, temp, pres, qv):
    ...
    tqp_data = np.swapaxes(tqp_data,1,2)
    tqp_data = torch.from_numpy(tqp_data).float()
    extra_data = torch.from_numpy(extra_data[:,0,:]).float()
    
    print("before cape_model prediction")
    print(tqp_data.shape, extra_data.shape)
    CAPE_model.eval()
    with torch.no_grad():
        BCU = np.sum(BCU_model(tqp_data,extra_data).numpy())
